import random
from collections import deque, OrderedDict
from typing import List

# Description: https://leetcode.com/problems/kth-largest-element-in-an-array/description/
class Solution_useSort:
    def findKthLargest(self, nums: List[int], k: int) -> int:
        nums.sort(reverse=True)
        return nums[k-1]

# Use Partition
# Very slow
# Description: https://leetcode.com/problems/kth-largest-element-in-an-array/description/
# Quick sort has different partition, use Hoare partition scheme partition!!!
class Solution_usePartition:
    def findKthLargest(self, nums: List[int], k: int) -> int:
        elementIndex = -1
        start = 0
        end = len(nums) -1
        k = len(nums) - k
        while start <= end:
            elementIndex = self.partition_Lomuto(nums, start, end)
            # print(nums)
            if elementIndex < k:
                start = elementIndex + 1
            elif elementIndex > k:
                end = elementIndex - 1
            else:
                break
        return nums[elementIndex]
    
    # Slow partition, not good at dealing with many duplicates in input
    def partition_Lomuto(self, nums: List[int], start: int, end: int) -> int:
        pivotIndex = random.randint(start, end)
        # print(f"Pivot index: {pivotIndex}")
        # pivotIndex = end
        pivot = nums[pivotIndex]
        print(f"Start calling: {start}, End: {end}, pivot: {pivot}, pivotIndex: {pivotIndex}")

        # Swap pivot and end
        temp = nums[end]
        nums[end] = nums[pivotIndex]
        nums[pivotIndex] = temp

        # all elements <= j are smaller than pivot
        j = start - 1
        for i in range(start, end):
            if nums[i] < pivot:
                j += 1
                (nums[i], nums[j]) = (nums[j], nums[i])
        (nums[j+1], nums[end]) = (nums[end], nums[j+1])
        return j+1

    # Still have bug, test input: [2,1,2]    
    def partition_Hoare(self, nums: List[int], start: int, end: int) -> int:
        pivotIndex = random.randint(start, end)
        pivot = nums[pivotIndex]
        print(f"Start calling: {start}, End: {end}, pivot: {pivot}, pivotIndex: {pivotIndex}")

        while start < end:
            while nums[start] <= pivot and start < end:
                start += 1
            while nums[end] >= pivot and start < end:
                end -= 1
            if start < end:
                nums[start], nums[end] = nums[end], nums[start]
                start += 1
                end -= 1

        print(f"Before return: {start}, End: {end}, pivot: {pivot}, pivotIndex: {pivotIndex}")
        return start

# Why this solution is faster than my partition solution?
class Solution_quickSelect:
    def findKthLargest(self, nums: List[int], k: int) -> int:
        k = len(nums) - k

        def quickSelect(left, right):            
            pivot = nums[right]
            low = left
            high = right

            while low <= high:
                while low <= high and nums[low] < pivot:
                    low += 1
                while low <= high and nums[high] > pivot:
                    high -= 1
                if low <= high:
                    nums[low], nums[high] = nums[high], nums[low]
                    low += 1
                    high -= 1

            if k <= high:
                return quickSelect(left, high)
            elif k >= low:
                return quickSelect(low, right)
            else:
                return nums[k]

        return quickSelect(0, len(nums) - 1)

if __name__ == "__main__":
    # Run the solution code here
    # pass
    nums = [2,1,2]
    k = 2
    s = Solution_quickSelect()
    print(f"result: {s.findKthLargest(nums, k)}")

    # s1 = Solution_quickSelect()
    # print(s1.findKthLargest(nums, k))